...
complete -c webwerk -f -n '__ww_mod_env; and not __fish_seen_subcommand_from theme plugin' -a theme -d 'Activate a theme: theme [webwerk|NAME|NUM]'
complete -c webwerk -f -n '__ww_mod_ctx; and __fish_seen_subcommand_from theme' -a webwerk -d 'Activate the webwerk theme'
complete -c webwerk -f -n '__ww_mod_env; and not __fish_seen_subcommand_from theme plugin' -a plugin -d 'Plugin actions: plugin <install|copy|update|activate|deactivate|remove|list>'
