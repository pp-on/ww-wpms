...
setup_nginx_config() {
    ...
    cat > nginx.conf << EOF
# WordPress Nginx Configuration - Generated by Webwerk
# Copy to /etc/nginx/sites-available/${CURRENT_DIR}.conf and enable:
#   sudo ln -s /etc/nginx/sites-available/${CURRENT_DIR}.conf /etc/nginx/sites-enabled/
