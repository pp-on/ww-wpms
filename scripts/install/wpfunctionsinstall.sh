...
# Create nginx config snippet (LEMP mode)
create_nginx_config_file() {
    log_info "Generating nginx config (LEMP mode, skipping .htaccess)"

    local project_name
    project_name="$(basename "$PWD")"
    local php_fpm="${PHP_FPM_SOCKET:-unix:/run/php/php-fpm.sock}"
    local max_body="${PHP_POST_MAX_SIZE:-${PHP_UPLOAD_MAX_FILESIZE:-64M}}"
    local server_name="${WP_URL#https://}"
    server_name="${server_name#http://}"
    server_name="${server_name%%/*}"

    local security_blocks=""
    if [[ "${NGINX_PRODUCTION:-false}" == "true" ]]; then
        security_blocks="
    location ~ /\.(ht|git) {
        deny all;
    }
    ...
    cat > nginx.conf << EOF
# WordPress Nginx Configuration - Generated by Webwerk v${SCRIPT_VERSION}
# $(date '+%Y-%m-%d %H:%M:%S')
# Copy to /etc/nginx/sites-available/${project_name}.conf and enable:
...
EOF
    ...
    chmod "${HTACCESS_FILE_PERMISSIONS:-644}" nginx.conf
    log_success "nginx.conf created for: $server_name"
    log_info "Adjust PHP_FPM_SOCKET in .env if your socket path differs (current: ${php_fpm})"
    return 0
}

# Dispatch to Apache (.htaccess) or nginx config based on WEBSERVER_TYPE
setup_webserver_config_file() {
...
install_full_wordpress() {
    ...
    setup_webserver_config_file || return 1
